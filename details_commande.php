  ...
  function detail(objects){
     ...
     $.ajax({
        ...
        $.each(tab, function (key, val) {
          $(".detail").append("<tr><td><img src='AdminLogin/images/"+val.imageArt+" ' style='width:130px;'><td>" + val.designation + "</td><td>" + val.prix1 + "</td></tr>");
        });
      }
      });
      ...
      // lors de chargement de la page effectué une requete http             
       $.ajax({
        ...
        $.each(response, function (key, val) {
          ...
          $("tbody:eq(1)").append(" <tr><td>" + val.idlistecom + "</td><td>" + val.nomClient + "</td>  <td>" + val.prenomClient + "</td>  <td><button type='button' onClick='valider(this)' class='btn btn-primary'>valider</button></td><td><button type='button' onClick='refuser(this)' class='btn btn-danger'>refuser</button></td><td><button type='button' onClick='detail(this)' class='btn btn-info' data-toggle='modal' data-target='#exampleModal'>detail</button></td></tr>");
