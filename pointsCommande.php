...
<body>
	...
	<script>
		...
		localStorage.removeItem("IdlistePoints");
		$(function () {
			// datatables---------------------------------------------
			$("#example").dataTable();
				...
				success: function (response) {
					if (JSON.parse(response).length > 0) {
						$.each(JSON.parse(response), function (key, val) {
							...
							t.row.add(["<img width='140px' src='AdminLogin/images/" + val.imageArt + "' />", val.designation, val.descriptions, val.date]).draw(false);
						});
					} else {
